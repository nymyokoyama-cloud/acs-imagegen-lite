...
```bash
docker build -t acs-imagegen-lite:0.3.0-rc5 .
docker run --gpus all -p 8080:8080 acs-imagegen-lite:0.3.0-rc5
```
...
公開前候補です。RC4でH200実機のH3 3動画モードを確認済みです。RC5ではモデル高速取得と終了結果表示を追加し、一般配布前の最終実機確認を行います。
